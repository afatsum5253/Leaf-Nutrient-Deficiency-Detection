...
# answer = y_pred
nit='NITROGEN Apply aged compost, manure, soybean meal or cottonseed meal to the soil once in spring. Seaweed extract will improve the soil environment thus giving nitrogen fixing bacteria a boost.'
nit=nit.encode()
pot='POTASSIUM Spray plant weekly with fish emulsion until symptoms quit.'
pot=pot.encode()
if y_pred=='nitrogen':
    conn.sendall(nit)
elif y_pred=='potassium':
    conn.sendall(pot)
else:
    conn.sendall(y_pred)
